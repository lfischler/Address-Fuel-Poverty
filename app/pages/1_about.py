...
The webapp uses the Scottish Government's publicly available Energy Performance Certificate dataset. This dataset contains the data from all energy performance certificates that have been carried out since 2012. An EPC must be completed when a building is bought, sold, or rented to a tenant.

We've mapped out the mean EPC score for each postcode. For more details on EPC scores please see the [Scottish Government Website](https://www.mygov.scot/energy-performance-certificates).

#### Scottish Index of Multiple Deprivation Dataset (SIMD)
The Scottish Index of Multiple Deprivation dataset is used as a measure of deprivation. It is a score assigned by the Scottish Government to analyse deprivation and its effects on households.
...
We use the two data sources above to give postcodes a unique fuel poverty risk rating. This risk rating is calculated by generating the mean of the percentile EPC rating and the percentile SIMD rating to directly combine the two biggest factors in fuel poverty.
